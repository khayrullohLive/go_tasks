...
type CommentRepository interface {
	// O'qish
	FindByPostID(postID uint, query models.CommentQuery) ([]models.Comment, int64, error) // ✅ query qo'shildi
...
// Post kommentariylarini olish (faqat top-level, replies bilan)
func (r *commentRepository) FindByPostID(postID uint, query models.CommentQuery) ([]models.Comment, int64, error) {
	...
	// Avval jami sonini hisoblash
	r.db.Model(&models.Comment{}).
		Where("post_id = ? AND parent_id IS NULL", postID).
		Count(&total)

	// Keyin sahifalab olish
	result := r.db.
		Where("post_id = ? AND parent_id IS NULL", postID).
		Preload("Author").
